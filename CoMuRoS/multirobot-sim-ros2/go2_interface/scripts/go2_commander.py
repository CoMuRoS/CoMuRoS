...
class Go2Interface(Node):
    def __init__(self):
        ...
        self.odom_subscriber = self.create_subscription(Odometry, '/odom/ground_truth', self.odom_callback, 10)
    ...
    #     self.publish_vel(x=1.0)
    #     start_time = time.time()
    #     while time.time() - start_time < (duration * 2.0) - 1.25:
    #         rclpy.spin_once(self, timeout_sec=0.05)
    #     self.publish_vel(0.0, 0.0, 0.0)
    #     self._task_done = True
    ...
    def go_to_position(self, x: float, y: float, z: float):
        ...
        self.target_z = z
        self.mode = MotionMode.GOTO
        self.goto_timer = self.create_timer(0.1 , self.goto_helper)

    def goto_helper(self):
        ...
        if abs(self.angle - self.current_ang_z) > 0.1: 
            ...
            if abs(self.current_x - self.target_x) < 0.3:  
